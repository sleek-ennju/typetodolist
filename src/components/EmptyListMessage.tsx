...
const EmptyListMessage = ({title, description}: emptyTodoList) => {
  return (
    <div className="border-t border-base-400 mt-6 flex flex-col justify-center items-center flex-1 gap-6 rounded-t-md">
        ...
        <p className="text-sm md:text-base text-center text-base-300 font-bold">{title}</p>
        <p className="text-sm md:text-base text-center text-base-300 font-normal">{description}</p>
        </div>
  </div>
  )
}
